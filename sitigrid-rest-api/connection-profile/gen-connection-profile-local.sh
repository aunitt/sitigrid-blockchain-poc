...
# This script uses the template sitigrid-connection-profile.json to generate a connection profile
# for the organisations in the Fabric network.

#REPODIR points to this repo.
REPODIR=~/Documents/apps/sitigrid-blockchain-poc
...
cp sitigrid-connection-profile.yaml $REPODIR/tmp/connection-profile
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    sed -e "s|%REPODIR%|$CERTDIR|g" sitigrid-connection-profile.yaml > $REPODIR/tmp/connection-profile/sitigrid-connection-profile.yaml
else
    sed -i "s|%REPODIR%|$CERTDIR|g"  $REPODIR/tmp/connection-profile/sitigrid-connection-profile.yaml
fi 
