...
echo Executing command: aws managedblockchain create-network --region $REGION \
    ...
    --name "${NETWORKNAME}" \
    --description "Sitigrid" Fabric network"
    ...
    --member-configuration "Name=\"${MEMBERNAME}\",Description=\"Sitigrid" Fabric member\",FrameworkConfiguration={Fabric={AdminUsername=${ADMINUSER},AdminPassword=${ADMINPWD}}}"
